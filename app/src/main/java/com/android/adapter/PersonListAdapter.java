...
public class PersonListAdapter extends BaseAdapter {
    ...
    public int getId(int position){
        int id = 0;
        try {
            id = mfollowJsons.get(position).getInt("id");
        } catch (Exception e) {
            e.printStackTrace();
        }
        return id;
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        try {
            ...
            //获取用户头像
            BitmapLoaderUtil.getInstance().getImage(viewHolder.userAvatar, BitmapLoaderUtil.TYPE_ORIGINAL, mfollowJsons.get(position).getInt("avatar"));
/*
